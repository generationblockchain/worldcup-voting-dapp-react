...
module.exports = async function(callback) { 
    ...
    await submitRandomVote(1, teams);
    await submitRandomVote(2, teams);
    await submitRandomVote(3, teams);
    await submitRandomVote(4, teams);
    await submitRandomVote(5, teams);
    await submitRandomVote(6, teams);
    await submitRandomVote(7, teams);
    await submitRandomVote(8, teams);
    await submitRandomVote(9, teams);

    console.log("\n\n");
    console.log(await fetchGameState());

    let winner = getRandomTeamName(teams);
    console.log(`\n\nAttempting to declare ${winner} winner!`);
...
async function submitRandomVote(fromAccountIndex, teams){
    let fromAccount = web3.eth.accounts[fromAccountIndex];
    let voteAmount = getRandomEtherAmount();
    let voteForTeam = getRandomTeamName(teams);
